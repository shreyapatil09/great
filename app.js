...
// ============ AUTH FLOW — BACKEND INTEGRATED ============
async function nextAuthStep() {
  const email = (document.getElementById('authEmail').value || '').trim();
  const pass = (document.getElementById('authPass').value || '').trim();
  if (!email || !pass) { shake(document.querySelector('.modal-box')); return; }

  // Admin shortcut (no backend call needed — admin is a special local role)
  ...
  if (preSelectedRole) {
    // Role already known — call API immediately
    try {
      await apiSignup(name, email, pass, preSelectedRole);
  ...
  // No role yet — collect role first, then sign up
  currentUser = { name, email, role: '', isNew: true };
  // Store pass temporarily so selectRole can call the API
  currentUser._pendingPass = pass;
...
async function selectRole(role) {
  ...
  const pass = currentUser._pendingPass || '';
  if (pass && currentUser.email && currentUser.name) {
    try {
      await apiSignup(currentUser.name, currentUser.email, pass, role);
      delete currentUser._pendingPass;
      saveUser(currentUser);
    } catch (err) {
      shake(document.querySelector('.modal-box'));
      showToast('⚠️ ' + err.message);
...
function renderAdminDashboard() {
  ...
  // For admin: load all users from localStorage (non-auth user list for display purposes)
  const users = _loadAllKnownUsers();
...
// Helper: load all locally tracked users (non-auth, for admin display)
function _loadAllKnownUsers() {
  try {
    // Collect unique emails from activity keys
    const users = [];
    for (let i = 0; i < localStorage.length; i++) {
      const key = localStorage.key(i);
      if (key && key.startsWith('st_activity_')) {
        const email = key.replace('st_activity_', '');
        if (email && email !== 'anon') users.push({ email });
      }
    }
    return users;
  } catch(e) { return []; }
...
function adminPayStudent(taskId) {
  ...
  const allUsers = _loadAllKnownUsers();
  allUsers.forEach(u => {
...
function renderAdminUsers() {
  ...
  adminRow.innerHTML = `
    ...
    <div class="admin-user-info"><strong>Admin</strong><span>admin@skillthali.com</span></div>
    <span class="admin-badge badge-purple">👑 Admin</span>
  `;
  container.appendChild(adminRow);
  users.forEach(u => {
    const row = document.createElement('div');
    row.className = 'admin-user-row';
    const earnings = loadStudentEarnings(u.email);
    row.innerHTML = `
      <div class="admin-user-avatar">${(u.email || 'U').substring(0,2).toUpperCase()}</div>
      <div class="admin-user-info"><strong>${u.email}</strong><span>${u.email}</span></div>
      <span class="admin-badge badge-green">🎓 User</span>
      ${earnings > 0 ? `<span class="admin-badge badge-green" style="margin-left:8px">💰 ₹${earnings.toLocaleString('en-IN')}</span>` : ''}
    `;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Wire up the Continue button via addEventListener (no inline onclick)
  const continueBtn = document.querySelector('#authStep1 .btn-primary');
  if (continueBtn) {
    continueBtn.removeAttribute('onclick');
    continueBtn.addEventListener('click', nextAuthStep);
  }

  // Session persistence — check localStorage for saved user (set by API login)
